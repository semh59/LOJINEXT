...
"""
        ...
        self.parser.add_argument("--timeout", type=int, default=300,
        ...
        self.parser.add_argument("--checks", type=str, default=None,
                                 help="Virgülle ayrılmış check listesi (örn: ai,weather,db)")
        self.parser.add_argument("--env", type=str, default="dev",
    ...
    def _check_interrupted_message(self):
        ...
        if self._interrupted and self._interrupt_signal is not None and not self.args.json:
            sig_name = "SIGINT" if self._interrupt_signal == signal.SIGINT else "SIGTERM"
    ...
    def run_with_timeout(self, func: Callable, timeout: Optional[int] = None, 
                          *args, **kwargs) -> Any:
        ...
        if thread.is_alive():
            ...
            raise TimeoutError(f"İşlem {timeout} saniye içinde tamamlanamadı (thread arka planda bırakıldı)")
    ...
    def run_with_retry(self, func: Callable, name: str, 
    ...
    def _finalize_json(self, total_duration: float, success_count: int, 
    ...
    def _finalize_human(self, total_duration: float, success_count: int, 
                        fail_count: int, skip_count: int):
