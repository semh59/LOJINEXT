...
class CorrelationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if correlation_id and not UUID4_PATTERN.match(correlation_id):
            logger.warning(f"Invalid correlation ID format received: {correlation_id[:50]}")
        ...
        # Debug log
        logger.debug(f"Request started: {request.method} {request.url.path} [cid={correlation_id[:8]}]")
