...
class ReportService:
    ...
    def __init__(
        self, sefer_repo=None, yakit_repo=None, arac_repo=None, sofor_repo=None
    ):
        if arac_repo:
    ...
    async def generate_vehicle_report(
        ...
        guzergahlar_task = self.analiz_repo.get_top_routes_by_vehicle(
            arac_id, start_date, limit=5
        )

        stats, gunluk, guzergahlar = await asyncio.gather(
            stats_task, gunluk_task, guzergahlar_task
        )
        ...
        return {
            ...
            "hedef_tuketim": arac.get(
                "hedef_tuketim", self.analiz_repo.DEFAULT_FILO_ORTALAMA
            ),
    ...
    async def generate_driver_report(self, sofor_id: int, days: int = 30) -> Dict:
        ...
        degerlendirme = await get_container().degerlendirme_service.evaluate_driver(
            sofor_id
        )
