...
class PDFReportGenerator:
    ...
    def _register_fonts(self):
        ...
        try:
            ...
            current_dir = os.path.dirname(os.path.abspath(__file__)) # app/core/services
            app_dir = os.path.dirname(os.path.dirname(current_dir)) # app
            asset_font = os.path.join(app_dir, "assets", "fonts", "EliteFont.ttf")
            asset_font_bold = os.path.join(app_dir, "assets", "fonts", "EliteFont-Bold.ttf")

            if os.path.exists(asset_font):
                pdfmetrics.registerFont(TTFont('EliteFont', asset_font))
                pdfmetrics.registerFont(TTFont('EliteFontBold', asset_font_bold if os.path.exists(asset_font_bold) else asset_font))
            ...
            if os.path.exists(font_path):
                ...
                pdfmetrics.registerFont(TTFont('EliteFontBold', font_path.replace('.ttf', 'bd.ttf') if 'bd.ttf' not in font_path else font_path))
                self.font_name = 'EliteFont'
                self.font_bold = 'EliteFontBold'
            else:
    ...
    def _create_header(self, elements: List, title: str, subtitle: Optional[str] = None):
    ...
    def _create_metric_box(self, label: str, value: str, color: Any = None) -> Table:
        ...
        data = [[Paragraph(f"<b>{label}</b>", self.styles['EliteBody']), 
                 Paragraph(f"<font color='{color.hexval() if color else '#000000'}'>{value}</font>", self.styles['EliteBody'])]]
    ...
    def generate_fleet_summary(self, start_date: date, end_date: date, data: Dict) -> bytes:
        ...
        # 1. Header
        self._create_header(elements, "Filo Performans Raporu", 
        ...
        m_data = [
            [self._create_metric_box("Toplam Araç", str(data.get('total_vehicles', 0))), 
             self._create_metric_box("Toplam Sefer", str(data.get('total_trips', 0)))],
            [self._create_metric_box("Toplam Mesafe", f"{data.get('total_distance', 0):,.0f} km"), 
             self._create_metric_box("Yakıt Tüketimi", f"{data.get('total_fuel', 0):,.0f} L")],
            [self._create_metric_box("Ort. Tüketim", f"{data.get('avg_consumption', 0):.2f} L/100km", self.SUCCESS), 
             self._create_metric_box("Toplam Maliyet", f"{data.get('total_cost', 0):,.2f} TL", self.PRIMARY)]
        ]
        metrics_grid = Table(m_data, colWidths=[8.5*cm, 8.5*cm])
        metrics_grid.setStyle(TableStyle([('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('TOPPADDING', (0, 0), (-1, -1), 5)]))
        ...
        if data.get('vehicle_performance'):
            ...
            v_table = Table(v_data, colWidths=[3*cm, 2*cm, 3*cm, 4*cm, 2*cm, 3*cm])
        ...
        elements.append(Paragraph(f"LojiNext AI Zekası Tarafından {datetime.now().strftime('%d.%m.%Y %H:%M')} tarihinde oluşturulmuştur.", 
                                 self.styles['EliteFooter']))

        doc.build(elements)
        return buffer.getvalue()

    def generate_vehicle_report(self, arac_id: int, month: int, year: int, data: Dict) -> bytes:
        ...
        self._create_header(elements, f"ARAÇ ANALİZ DOSYASI: {plaka}", f"Rapor Dönemi: {month:02d}/{year}")
        ...
        t_table.setStyle(TableStyle([('BACKGROUND', (0, 0), (0, -1), self.BG_LIGHT), ('GRID', (0, 0), (-1, -1), 0.5, self.SECONDARY)]))
    ...
    async def async_generate_fleet_summary(self, start_date: date, end_date: date, data: Dict) -> bytes:
        """Asenkron wrapper: Filo Özet Raporu"""
        import asyncio
        return await asyncio.to_thread(self.generate_fleet_summary, start_date, end_date, data)

    async def async_generate_vehicle_report(self, arac_id: int, month: int, year: int, data: Dict) -> bytes:
        """Asenkron wrapper: Araç Detay Raporu"""
        import asyncio
        return await asyncio.to_thread(self.generate_vehicle_report, arac_id, month, year, data)
