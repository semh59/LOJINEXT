...
class TimeSeriesPredictor:
    ...
    def prepare_features(self, daily_data: List[Dict]) -> np.ndarray:
        ...
        for i, day in enumerate(daily_data):
            ...
            # Lag özellikleri
            prev_consumption = daily_data[i-1].get('ort_tuketim', ort_tuketim) if i > 0 else ort_tuketim
            ...
            if i >= 7:
                first_half_vals = [daily_data[j].get('ort_tuketim', 32.0) for j in range(i-6, i-3)]
                second_half_vals = [daily_data[j].get('ort_tuketim', 32.0) for j in range(i-3, i+1)]
    ...
    def create_sequences(
        ...
        if n_days < required_days:
            ...
            padding_X = np.zeros((padding_size, features.shape[1]), dtype=features.dtype)
            ...
            logger.warning(f"TimeSeries: Yetersiz veri ({n_days} gün). {padding_size} günlük zero-padding uygulandı.")

        for i in range(len(features) - self.SEQUENCE_LENGTH - self.FORECAST_DAYS + 1):
            X.append(features[i:i + self.SEQUENCE_LENGTH])
            y.append(targets[i + self.SEQUENCE_LENGTH:i + self.SEQUENCE_LENGTH + self.FORECAST_DAYS])
    ...
    def normalize(self, X: np.ndarray, y: np.ndarray = None, fit: bool = True):
        ...
        if fit:
            ...
            if y is not None:
                ...
                # NaN koruması
                self.target_mean = 0.0 if not np.isfinite(self.target_mean) else self.target_mean
                self.target_std = 1.0 if not np.isfinite(self.target_std) or self.target_std <= 1e-10 else self.target_std
    ...
    def train(
        ...
        try:
            ...
            targets = np.array([d.get('ort_tuketim', 32.0) for d in daily_data], dtype=np.float32)
    ...
    def predict(
        ...
        return TimeSeriesPrediction(
            ...
            trend=trend,
            model_accuracy=self.training_history[-1]['val_loss'] if self.training_history else 0.0,
    ...
    def save_model(self, filepath: str):
        ...
        logger.info(f"LSTM model saved (hybrid) to {base_path} with checksum {weights_checksum[:16]}...")
    ...
    def load_model(self, filepath: str):
        ...
        if expected_checksum:
            ...
            if actual_checksum != expected_checksum:
                logger.error(f"GÜVENLİK İHLALİ: Model checksum uyuşmazlığı! Beklenen: {expected_checksum[:16]}..., Gerçek: {actual_checksum[:16]}...")
                raise RuntimeError("Security: Model dosyası bozulmuş veya değiştirilmiş olabilir!")
            logger.debug(f"Model checksum doğrulandı: {actual_checksum[:16]}...")
        else:
            logger.warning(f"Model yüklendi ancak checksum doğrulaması yapılamadı (eski format)")
            
        self.feature_mean = np.array(metadata['feature_mean'], dtype=np.float32) if metadata['feature_mean'] else None
        self.feature_std = np.array(metadata['feature_std'], dtype=np.float32) if metadata['feature_std'] else None
        ...
        try:
            if use_weights_only:
                ...
                logger.warning("Eski PyTorch sürümü tespit edildi, weights_only=True kullanılamıyor.")
        ...
        logger.info(f"LSTM model loaded (hybrid) from {base_path} (weights_only={use_weights_only}, verified={bool(expected_checksum)})")
