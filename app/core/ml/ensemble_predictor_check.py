    ...
    def fit(self, seferler: List[Dict], y_actual: np.ndarray) -> Dict:
        ...
        try:
            ...
            with self._model_lock:
                ...
                # ML modelleri eğit (Training set üzerinde)
                ...
                # ---------------------------------------------------------
                # DYNAMIC WEIGHTING ALGORITHM (Faz 2)
                # ---------------------------------------------------------
                # 1. Pozitif R2 skoru olan modelleri belirle
                r2_scores = {
                    "gb": max(0, gb_test_r2),
                    "rf": max(0, rf_test_r2),
                    "xgboost": max(0, xgb_r2) if xgb_r2 else 0,
                    "lightgbm": max(0, lgb_r2) if lgb_r2 else 0,
                }

                # 2. Fizik modelinin ağırlığını sabitle veya performansa göre ayarla
                # Fizik modeli her zaman base olduğu için minimum bir ağırlık veriyoruz (%10)
                # Ancak ML modelleri çok kötüyse (R2 <= 0), fizik ağırlığı artar.
                base_physics_weight = 0.10
                ml_total_r2 = sum(r2_scores.values())

                new_weights = {}
                if ml_total_r2 > 0:
                    # ML modelleri başarılı, R2 oranına göre dağıt (kalan %90 payı)
                    ml_share = 1.0 - base_physics_weight
                    new_weights["physics"] = base_physics_weight

                    for model, score in r2_scores.items():
                        if score > 0:
                            weight = (score / ml_total_r2) * ml_share
...
class EnsemblePredictorService:
    ...
    async def predict_consumption(
        ...
        if sofor_id:
            sofor_service = get_sofor_analiz_service()
            # Note: sofor_service.get_driver_stats currently manage its own sessions
            # In a full-scale refactor, we would pass 'uow' here too
