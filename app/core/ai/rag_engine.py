...
class FAISSVectorStore:
    ...
    def add(
        ...
        if self.count() >= self.MAX_INDEX_SIZE:
            logger.error(
                f"Index full! Max size: {self.MAX_INDEX_SIZE}. Rejecting: {doc_id}"
            )
            return
...
class RAGEngine:
    ...
    EMBEDDING_DIM = 1024  # Unified for BGE-M3
    RAG_MAX_CHARS = 4000
    SIMILARITY_THRESHOLD = 0.35  # Slightly lowered for BGE-M3 density

    def __init__(self):
        self.embedder = None
        self.vector_store = None
        self.is_initialized = False
        self.status = "offline"  # offline, loading, ready, error
        self._init_lock = threading.Lock()  # Thread-safe init guard
        self._last_inference_time_ms: float = 0.0  # Performance tracking

        if not self.EMBEDDING_MODEL:
            self.EMBEDDING_MODEL = settings.AI_EMBEDDING_MODEL
            self.EMBEDDING_DIM = settings.AI_EMBEDDING_DIM
            self.RAG_MAX_CHARS = settings.AI_RAG_MAX_CHARS
            self.SIMILARITY_THRESHOLD = settings.AI_RAG_THRESHOLD
            self.MAX_DOCUMENT_CHARS = settings.AI_RAG_MAX_DOC_CHARS
