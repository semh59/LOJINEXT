...
from app.core.ai.rag_engine import RAGEngine, FAISSVectorStore
...
@pytest.fixture
def mock_rag_engine():
    with patch("app.core.ai.rag_engine.SENTENCE_TRANSFORMERS_AVAILABLE", True), \
         patch("app.core.ai.rag_engine.FAISS_AVAILABLE", True), \
         patch("app.core.ai.rag_engine.faiss") as mock_faiss: # Mock the faiss module
...
class TestModelSecurity:
    ...
    def test_untrusted_model_id(self):
        ...
        with patch.dict(os.environ, {"AI_MODEL_ID": "hacker/malicious-model"}):
            chatbot = QwenChatbot(load_model=False)
            assert chatbot.MODEL_ID == "Qwen/Qwen2.5-1.5B-Instruct", "Should revert to default"
...
class TestRAGRobustness:
    ...
    async def test_top_k_limit(self, mock_rag_engine):
        ...
        with patch.object(mock_rag_engine.vector_store, 'search', return_value=[]) as mock_search:
