...
class TestAnomalyDetector:
    ...
    @pytest.mark.asyncio
    async def test_detect_consumption_anomalies_with_outlier(self, detector, sample_consumption_data):
...
class TestPhysicsFuelPredictor:
    ...
    def test_predict_heavier_load_consumes_more(self, predictor):
        ...
        light_load = RouteConditions(
            distance_km=100,
            load_ton=10.0,
            avg_speed_kmh=80
        )
        
        heavy_load = RouteConditions(
            distance_km=100,
            load_ton=26.0,
            avg_speed_kmh=80
        )
    ...
    def test_predict_higher_speed_consumes_more(self, predictor):
        ...
        slow_speed = RouteConditions(
            distance_km=100,
            load_ton=20.0,
            avg_speed_kmh=60
        )
        
        fast_speed = RouteConditions(
            distance_km=100,
            load_ton=20.0,
            avg_speed_kmh=100
        )
...
class TestEnsemblePredictor:
    ...
    def sample_seferler(self):
        ...
        return [
            {
                'mesafe_km': 450, 'ton': 20.0, 'ascent_m': 500, 'descent_m': 300,
                'sofor_score': 0.95, 'tuketim': 34.5
            },
            {
                'mesafe_km': 300, 'ton': 15.0, 'ascent_m': 200, 'descent_m': 250,
                'sofor_score': 0.90, 'tuketim': 32.0
            },
            {
                'mesafe_km': 600, 'ton': 25.0, 'ascent_m': 800, 'descent_m': 400,
                'sofor_score': 1.0, 'tuketim': 38.0
            },
            {
                'mesafe_km': 200, 'ton': 10.0, 'ascent_m': 100, 'descent_m': 150,
                'sofor_score': 0.85, 'tuketim': 30.5
            },
            {
                'mesafe_km': 500, 'ton': 22.0, 'ascent_m': 600, 'descent_m': 350,
                'sofor_score': 0.92, 'tuketim': 35.8
            },
...
class TestRAGEngine:
    ...
    def rag_engine(self):
        ...
        try:
            ...
            if not is_rag_available():
                pytest.skip("RAG dependencies not available (FAISS/SentenceTransformer)")
    ...
    @pytest.mark.asyncio
    async def test_search_for_context_max_chars_limit(self, rag_engine, sample_vehicle_data):
...
class TestWeatherService:
    ...
    async def test_get_weather_valid_coords(self, weather_service):
        ...
        with patch.object(weather_service.external_service, 'get_weather_forecast', new_callable=AsyncMock) as mock_fetch:
...
class TestInsightEngine:
    ...
    async def test_insight_structure(self, insight_engine):
        ...
        with patch('app.core.services.insight_engine.get_uow') as mock_uow_func:
            ...
            mock_uow.analiz_repo = AsyncMock()
            mock_uow.analiz_repo.get_all_vehicles_consumption_stats.return_value = [
                {'arac_id': 1, 'plaka': '34 ABC 123', 'hedef_tuketim': 32.0, 'ort_tuketim': 40.0}
            ]
            mock_uow_func.return_value = mock_uow
...
class TestTimeSeriesService:
    ...
    async def test_predict_weekly_no_model(self, ts_service):
        ...
        assert 'error' in result or result.get('success') == False or 'forecast' in result
    
    @pytest.mark.asyncio
    async def test_get_trend_analysis(self, ts_service):
        """Trend analizi"""
        # get_daily_summary metodunu mock'la (doğru format: dict listesi)
        with patch.object(ts_service, 'get_daily_summary', new_callable=AsyncMock) as mock:
            mock.return_value = [
                {'tarih': '2024-01-01', 'ort_tuketim': 32.0, 'toplam_km': 100, 'ort_ton': 10, 'sefer_sayisi': 5},
                {'tarih': '2024-01-02', 'ort_tuketim': 32.5, 'toplam_km': 110, 'ort_ton': 11, 'sefer_sayisi': 6},
                {'tarih': '2024-01-03', 'ort_tuketim': 33.0, 'toplam_km': 120, 'ort_ton': 12, 'sefer_sayisi': 5},
                {'tarih': '2024-01-04', 'ort_tuketim': 33.5, 'toplam_km': 115, 'ort_ton': 11, 'sefer_sayisi': 6},
                {'tarih': '2024-01-05', 'ort_tuketim': 34.0, 'toplam_km': 130, 'ort_ton': 13, 'sefer_sayisi': 7},
                {'tarih': '2024-01-06', 'ort_tuketim': 34.5, 'toplam_km': 125, 'ort_ton': 12, 'sefer_sayisi': 5},
                {'tarih': '2024-01-07', 'ort_tuketim': 35.0, 'toplam_km': 140, 'ort_ton': 14, 'sefer_sayisi': 8},
            ]
...
class TestEdgeCases:
    ...
    def test_physics_predictor_extreme_values(self):
        ...
        from app.core.ml.physics_fuel_predictor import PhysicsBasedFuelPredictor, VehicleSpecs, RouteConditions
