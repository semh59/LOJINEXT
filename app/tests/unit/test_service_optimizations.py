...
from app.core.services.insight_engine import InsightEngine, Insight, InsightType
...
async def test_dashboard_service_parallelism():
    ...
    with patch("app.core.services.dashboard_service.get_analiz_repo") as mock_get_repo:
        ...
        async def delayed_return_stats():
            await asyncio.sleep(0.1)
            return {"stats": "ok"}

        async def delayed_return_trips(**kwargs):
            await asyncio.sleep(0.1)
            return []
        ...
        assert (end - start) < 0.18, "Dashboard data fetch should be parallel"
...
async def test_insight_engine_bulk_insert():
    ...
    # Mock dependencies
    engine.generate_fleet_insights = AsyncMock(return_value=[
        Insight(InsightType.UYARI, "filo", None, "Test Fleet")
    ])
    engine.generate_vehicle_insights_bulk = AsyncMock(return_value=[
        Insight(InsightType.UYARI, "arac", 1, "Test Vehicle")
    ])
    engine.generate_driver_insights_bulk = AsyncMock(return_value=[])
    
    with patch("app.database.repositories.analiz_repo.get_analiz_repo") as mock_get_repo:
        mock_repo = AsyncMock()
