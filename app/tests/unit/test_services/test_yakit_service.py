...
class TestYakitService:
    ...
    def test_service_singleton(self, yakit_service):
        ...
        assert yakit_service is service2, "get_yakit_service should return the same instance"
    ...
    async def test_get_by_vehicle_id(self, yakit_service):
        ...
        for record in filtered:
            r_arac_id = record.arac_id if hasattr(record, 'arac_id') else record.get('arac_id')
            assert r_arac_id == vehicle_id, f"Record should belong to vehicle {vehicle_id}"
...
class TestYakitServiceValidation:
    ...
    @pytest.mark.asyncio
    async def test_add_fuel_requires_positive_litre(self, yakit_service, sample_yakit_data):
...
class TestYakitServiceStats:
    ...
    async def test_get_yakit_stats(self, yakit_service, monkeypatch):
        ...
        monkeypatch.setattr("app.core.services.yakit_service.get_yakit_repo", lambda: MagicMock())
        monkeypatch.setattr("app.database.repositories.analiz_repo.get_analiz_repo", lambda: mock_analiz)
    ...
    async def test_get_monthly_summary(self, yakit_service, monkeypatch):
        ...
        mock_analiz = AsyncMock()
        mock_analiz.get_monthly_consumption_series.return_value = [{'month': 'Oca', 'consumption': 1000}]
        monkeypatch.setattr("app.database.repositories.analiz_repo.get_analiz_repo", lambda: mock_analiz)
