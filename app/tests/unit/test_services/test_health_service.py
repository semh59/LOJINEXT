...
async def test_check_db_healthy(health_service):
    ...
    with patch("app.core.services.health_service.AsyncSessionLocal") as mock_session_cls:
...
async def test_get_full_status(health_service):
    ...
    with patch.object(health_service, 'check_db', return_value={"status": "healthy"}) as mock_db, \
         patch.object(health_service, 'check_ai_readiness', return_value={"status": "healthy"}) as mock_ai:
