...
from app.core.ai.qwen_chatbot import get_chatbot, ChatMessage
...
async def test_chatbot_history_privacy():
    ...
    with patch.object(chatbot, 'model_loaded', True), \
...
def test_config_robustness():
    ...
    with patch.dict(os.environ, {"AI_MAX_HISTORY": "invalid_number", "AI_RAG_THRESHOLD": "not_a_float"}):
        chatbot = get_chatbot(load_model=False)
...
async def test_rag_thread_safe_init():
    ...
    with patch('app.core.ai.rag_engine.SentenceTransformer'), \
