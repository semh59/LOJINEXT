...
async def test_update_modifies_data(test_repo):
    ...
    new_id = await test_repo.create(plaka="34 UT 03", marka="Original", model="X", yil=2020)
...
async def test_get_all_filters_inactive(test_repo):
    ...
    del_id = await test_repo.create(plaka="34 UT 06", aktif=True, marka="X", model="M", yil=2000)
...
async def test_sql_injection_safe(test_repo, db_session):
    ...
    new_id = await test_repo.create(plaka=malicious_name, marka="Test", model="X", yil=2020)
    
    result = await test_repo.get_by_id(new_id)
    assert result["plaka"] == malicious_name
    
    # Check table still exists
    res = await db_session.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='araclar'"))
