...
from app.core.ml.physics_fuel_predictor import VehicleSpecs, PhysicsBasedFuelPredictor
from app.core.ml.kalman_estimator import KalmanFuelEstimator, KalmanState
...
class TestModelSerialization:
                ...
                if "pickle.load" in content:
                    ...
                    for i, line in enumerate(lines):
                        if "pickle.load" in line and not line.strip().startswith(("#", '"', "'")):
                             pytest.fail(f"Unsafe pickle.load found in {file.name} at line {i+1}")
    ...
    def test_torch_load_safety(self):
        ...
        ts_file = Path(__file__).parent.parent.parent.parent / "app" / "core" / "ml" / "time_series_predictor.py"
        with open(ts_file, "r", encoding="utf-8") as f:
             content = f.read()
             assert "weights_only=True" in content, "time_series_predictor.py must use weights_only=True for torch.load"
...
class TestNumericalStability:
    ...
    def test_physics_params_validation(self):
        ...
        with pytest.raises(ValueError, match="Engine efficiency must be greater than 0"):
    ...
    def test_benchmark_division_by_zero(self):
        ...
        res = ab.run_ab_test(
            "Zero Test", "A", np.array([10.0]), "B", np.array([10.0]), 
            actuals=np.array([10.0]), metric='MAE'
        ) 
        # Both MAE 0. 
