...
class TestInputValidation:
    ...
    async def test_sql_injection_attempt(self, async_client, auth_headers):
        ...
        response = await async_client.get("/api/v1/vehicles/1;DROP TABLE users", headers=auth_headers)
        assert response.status_code in [404, 422]
...
class TestRateLimiting:
    ...
    async def test_rate_limit_enforced_trips(self, async_client, auth_headers):
        ...
        # Create Vehicle
        vehicle_payload = {"plaka": "34TST99", "marka": "Test", "model": "Tir", "yil": 2023}
        resp_v = await async_client.post("/api/v1/vehicles/", json=vehicle_payload, headers=auth_headers)
        ...
        resp_d = await async_client.post("/api/v1/users/", json=driver_payload, headers=auth_headers) 
        ...
        payload = {
            "arac_id": v_id, "sofor_id": 1, "mesafe_km": 100, 
            "tuketim": 30.0, "tarih": "2026-01-01", 
            "cikis_yeri": "A", "varis_yeri": "B"
        }
        
        responses = []
        for _ in range(10):
            # We don't care about success, just hit count.
            resp = await async_client.post("/api/v1/trips/", json=payload, headers=auth_headers)
