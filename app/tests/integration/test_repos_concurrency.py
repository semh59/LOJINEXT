...
async def test_arac_repo_toctou_concurrency(async_db_engine):
    ...
    for r in results:
        if isinstance(r, int):
            ...
            error_details.append(f"Unexpected: {type(r).__name__}: {str(r)}")
    ...
    assert success_count == 1, f"Expected 1 success, got {success_count}. Errors: {error_details}"
    assert conflict_count == 1, f"Expected 1 conflict, got {conflict_count}. Errors: {error_details}"
