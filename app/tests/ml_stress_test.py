...
def test_memory_lru_eviction():
    ...
    if len(service.predictors) <= service.MAX_PREDICTORS:
        print("✅ BAŞARILI: LRU Cache limiti aşılmadı. Eski araçlar tahliye edildi.")
...
def test_kalman_stability_stress():
    ...
    for i in range(iterations):
        features = {
            "ton": 25 + np.random.normal(0, 5),
            "ascent_m": 100 + np.random.normal(0, 50),
            "arac_yasi": 5,
        }
    ...
    print(
        f"Süre: {duration:.2f} saniye. Ortalama: {duration / iterations * 1000:.4f} ms/iter"
    )
...
def test_statistical_selectivity():
    ...
    result_normal = ab_test.run_ab_test(
        "Normal_Test", "ModelA", pred_a, "ModelB", pred_b, actuals
    )
    ...
    result_outlier = ab_test.run_ab_test(
        "Outlier_Test", "ModelA", pred_a_out, "ModelB", pred_b_out, actuals
    )
