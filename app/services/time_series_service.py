...
class TimeSeriesService:
    ...
    async def get_daily_summary(
        ...
        try:
            rows = await analiz_repo.get_daily_summary_for_ml(days=days, arac_id=arac_id)
            return [
                {
                    "tarih": row.get("tarih"),
    ...
    def _filter_outliers(self, data: List[Dict], threshold: float = 3.0) -> List[Dict]:
        ...
        if len(data) - len(filtered) > 0:
            logger.info(f"Filtered {len(data) - len(filtered)} outliers from time series data.")
    ...
    async def train_model(
        ...
        result = await asyncio.to_thread(self.predictor.train, daily_data=daily_data, epochs=epochs)
        
        if result['success']:
            logger.info(f"Time series model trained for {'vehicle ' + str(arac_id) if arac_id else 'fleet'}")
        return result
    ...
    async def predict_weekly(
        ...
        if len(daily_data) < 30 and arac_id is not None:
            logger.info(f"Insufficient data for vehicle {arac_id}, falling back to fleet-wide model.")
            return await self.predict_weekly(arac_id=None)
            
        if len(daily_data) < 30:
            return {'success': False, 'error': 'Yetersiz veri (Filo geneli dahi).'}
    ...
    async def get_trend_analysis(
        ...
        if len(daily_data) < 7:
            return {
                'success': False,
                'error': 'Trend analizi için yetersiz veri'
            }
        ...
        ma7 = np.convolve(y, np.ones(7)/7, mode='valid').tolist() if len(y) >= 7 else []
        ...
        return {
            ...
            'trend_tr': trend_tr,
            'slope': round(slope, 4),
            'current_avg': round(np.mean(consumptions[-7:]), 2),
            'previous_avg': round(np.mean(consumptions[:7]), 2) if len(consumptions) >= 14 else None,
    ...
    def get_model_status(self) -> Dict:
        ...
        return {
            ...
            'training_epochs': len(self.predictor.training_history) if is_lstm_available() and self.predictor.training_history else 0,
            'last_loss': self.predictor.training_history[-1]['val_loss'] if is_lstm_available() and self.predictor.training_history else None
        }
