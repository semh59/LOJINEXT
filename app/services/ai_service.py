...
class LocalAIService:
    ...
    def _initialize_model(self):
        ...
        if not model_path.exists():
            ...
            if not self._verify_model_checksum(str(model_path)):
                logger.warning(
                    "Existing model checksum failed! Deleting and redownloading..."
                )
                model_path.unlink()
                try:
                    self._download_model(str(model_path))
    ...
    def _verify_model_checksum(self, path: str) -> bool:
        ...
        if not expected_hash:
            logger.warning(
                "AI_MODEL_SHA256 env not set, skipping checksum verification"
            )
        ...
        if actual_hash != expected_hash:
            logger.error(
                f"Model checksum mismatch! Expected {expected_hash[:16]}..., got {actual_hash[:16]}..."
            )
            return False
    ...
    def _download_model(self, dest_path: str):
        ...
        if not expected_hash:
            ...
            self._download_status["status"] = "error"
            raise SecurityError(
                "AI_MODEL_SHA256 env tanımlanmadı. Model indirme iptal."
            )
