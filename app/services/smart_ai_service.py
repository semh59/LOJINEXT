...
class KnowledgeBase:
    ...
    async def add_document(self, content: str, category: str, metadata: Dict = None) -> bool:
        ...
        embedding = await asyncio.to_thread(self.model.encode, content, convert_to_numpy=True)
        
        metadata = metadata or {}
        metadata.update({"category": category, "created_at": datetime.now().isoformat()})
    ...
    async def search(self, query: str, top_k: int = 5, category: str = None) -> List[Dict]:
        ...
        raw_results = await asyncio.to_thread(self.vector_store.search, query_embedding, top_k, source_types)
        ...
        for idx, score in raw_results:
            if score > 0.3:
                results.append({
                    ...
                    "category": self.vector_store.metadatas.get(idx, {}).get("category"),
...
class SmartAIService:
    ...
    async def learn_from_anomaly(self, anomaly_data: Dict) -> bool:
        """Anomali verisinden öğren (async)"""
        content = (
            f"Anomali Tespit: {anomaly_data.get('tip', '')}. "
            f"Sapma: %{anomaly_data.get('sapma_yuzde', 0):.1f}. "
            f"Açıklama: {anomaly_data.get('aciklama', '')}. "
            f"Önem: {anomaly_data.get('severity', 'medium')}."
        )

        return await self.kb.add_document(content, category="anomali", metadata=anomaly_data)
