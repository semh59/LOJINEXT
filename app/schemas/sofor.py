...
class SoforBase(BaseModel):
    ...
    ehliyet_sinifi: Literal["B", "C", "D", "E", "G"] = Field("E")
    score: float = Field(1.0, ge=0.1, le=2.0, description="Sonuç puanı (0.1-2.0)")
    manual_score: float = Field(1.0, ge=0.1, le=2.0, description="Manuel değerlendirme puanı")
    aktif: bool = True
...
class SoforUpdate(BaseModel):
    ...
    ise_baslama: Optional[date] = None
    ehliyet_sinifi: Optional[Literal["B", "C", "D", "E", "G"]] = None
...
class SoforResponse(SoforBase):
    ...
    def heal_name(cls, v: Any) -> str:
        ...
        if not v: return "İSİMSİZ SÜRÜCÜ"
        name = str(v).strip()
        if len(name) < 3: return f"{name} (Kısa İsim)"
        return name
