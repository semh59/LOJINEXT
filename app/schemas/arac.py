...
class AracBase(BaseModel):
    ...
    plaka: str = Field(
        ...
        min_length=3, 
        max_length=20, 
        pattern=r"^[0-9A-Z]{1,5}\s?[0-9A-Z]{1,5}\s?[0-9A-Z]{1,5}$",
    ...
    tank_kapasitesi: int = Field(600, gt=0, le=5000, description="Tank kapasitesi (litre)")
    hedef_tuketim: float = Field(32.0, gt=0, le=100, description="Hedef tüketim (lt/100km)")
    
    # Elite Technical Specs (Aerodynamics)
    bos_agirlik_kg: float = Field(8000.0, gt=0, le=40000, description="Boş Ağırlık (kg)")
    hava_direnc_katsayisi: float = Field(0.7, gt=0.1, le=2.0, description="Hava Direnç Katsayısı (Cd)")
    on_kesit_alani_m2: float = Field(8.5, gt=1.0, le=20.0, description="Ön Kesit Alanı (m2)")
    motor_verimliligi: float = Field(0.38, gt=0.1, le=1.0, description="Motor Verimliliği (0-1 arası)")
    lastik_direnc_katsayisi: float = Field(0.007, gt=0.001, le=0.1, description="Lastik Direnç Katsayısı (Crr)")
    maks_yuk_kapasitesi_kg: int = Field(26000, gt=0, le=50000, description="Maksimum Yük Kapasitesi (kg)")
...
class AracResponse(AracBase):
    ...
    id: int
    plaka: str = Field(..., description="Türkiye plaka formatı (Permissive in response)")
    created_at: datetime
