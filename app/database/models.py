...
class Arac(Base):
    ...
    tank_kapasitesi: Mapped[int] = mapped_column(Integer, server_default=text("600"), default=600)
    hedef_tuketim: Mapped[float] = mapped_column(Float, server_default=text("32.0"), default=32.0)
    
    # Elite Technical Specs
    bos_agirlik_kg: Mapped[float] = mapped_column(Float, server_default=text("8000.0"), default=8000.0)
    hava_direnc_katsayisi: Mapped[float] = mapped_column(Float, server_default=text("0.7"), default=0.7) # Cd
    on_kesit_alani_m2: Mapped[float] = mapped_column(Float, server_default=text("8.5"), default=8.5) # Frontal Area
    motor_verimliligi: Mapped[float] = mapped_column(Float, server_default=text("0.38"), default=0.38)
    lastik_direnc_katsayisi: Mapped[float] = mapped_column(Float, server_default=text("0.007"), default=0.007)
    maks_yuk_kapasitesi_kg: Mapped[int] = mapped_column(Integer, server_default=text("26000"), default=26000)
    
    aktif: Mapped[bool] = mapped_column(Boolean, default=True)
    notlar: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), default=datetime.now)

    # Relationships
    yakit_alimlari: Mapped[List["YakitAlimi"]] = relationship(back_populates="arac", cascade="all, delete-orphan")
    seferler: Mapped[List["Sefer"]] = relationship(back_populates="arac")
    yakit_periyotlari: Mapped[List["YakitPeriyodu"]] = relationship(back_populates="arac")
    formul: Mapped[Optional["YakitFormul"]] = relationship(back_populates="arac", uselist=False)
...
class Sofor(Base):
    ...
    score: Mapped[float] = mapped_column(Float, default=1.0) # Driver performance score (0.1 - 2.0)
...
class YakitAlimi(Base):
    ...
    arac_id: Mapped[int] = mapped_column(ForeignKey("araclar.id", ondelete="RESTRICT"), index=True)
    ...
    durum: Mapped[str] = mapped_column(String(20), default='Bekliyor') # Bekliyor, Onaylandi
    aktif: Mapped[bool] = mapped_column(Boolean, default=True)  # Soft delete flag
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), default=datetime.now)
    ...
    last_fetched: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), onupdate=func.now())
...
class Guzergah(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    ad: Mapped[str] = mapped_column(String(100), unique=True, index=True) # Örn: "Ankara - İstanbul (Standart)"
    cikis_yeri: Mapped[str] = mapped_column(String(100))
    varis_yeri: Mapped[str] = mapped_column(String(100))
    mesafe_km: Mapped[int] = mapped_column(Integer)
    
    # Defaults
    varsayilan_arac_id: Mapped[Optional[int]] = mapped_column(ForeignKey("araclar.id", ondelete="SET NULL"))
    varsayilan_sofor_id: Mapped[Optional[int]] = mapped_column(ForeignKey("soforler.id", ondelete="SET NULL"))
...
class Sefer(Base):
    ...
    # Foreign Keys
    guzergah_id: Mapped[Optional[int]] = mapped_column(ForeignKey("guzergahlar.id", ondelete="SET NULL"), index=True)
    arac_id: Mapped[int] = mapped_column(ForeignKey("araclar.id", ondelete="RESTRICT"), index=True)
    sofor_id: Mapped[int] = mapped_column(ForeignKey("soforler.id", ondelete="RESTRICT"), index=True)
    periyot_id: Mapped[Optional[int]] = mapped_column(Integer, index=True) # Soft link to periyot
    
    # Weight Info
    bos_agirlik_kg: Mapped[int] = mapped_column(Integer, default=0)
    dolu_agirlik_kg: Mapped[int] = mapped_column(Integer, default=0)
    net_kg: Mapped[int] = mapped_column(Integer, default=0) # Computed: Dolu - Boş
    ton: Mapped[float] = mapped_column(Float, default=0.0) # Computed
    ...
    tahmini_tuketim: Mapped[Optional[float]] = mapped_column(Float) # AI predicted consumption
...
class YakitPeriyodu(Base):
    ...
    alim1_id: Mapped[int] = mapped_column(Integer) # FK to YakitAlimi but handled manually or separate rel
...
class YakitFormul(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    arac_id: Mapped[int] = mapped_column(ForeignKey("araclar.id", ondelete="RESTRICT"), unique=True)
...
class Alert(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    alert_type: Mapped[str] = mapped_column(String(50), index=True)  # fuel_anomaly, cost_anomaly, driver_alert, vehicle_alert, system
    severity: Mapped[str] = mapped_column(String(20), default='medium')  # low, medium, high, critical
    title: Mapped[str] = mapped_column(String(200))
    message: Mapped[str] = mapped_column(Text)
    status: Mapped[str] = mapped_column(String(20), default='unread', index=True)  # unread, read, dismissed
    source_id: Mapped[Optional[int]] = mapped_column(Integer)  # İlgili kaydın ID'si
    source_type: Mapped[Optional[str]] = mapped_column(String(50))  # arac, sofor, sefer, yakit
...
class RoutePath(Base):
    ...
    __table_args__ = (
        UniqueConstraint('origin_lat', 'origin_lon', 'dest_lat', 'dest_lon', name='uq_route_coords'),
    )
    ...
    fuel_estimate_cache: Mapped[Optional[dict]] = mapped_column(JSON) # Tahmin sonucu önbelleği
