...
from app.schemas.sofor import SoforCreate, SoforResponse, SoforUpdate
...
@router.post("/", response_model=SoforResponse)
async def create_sofor(
    ...
    try:
        ...
        if not created_sofor:
            # Should not happen
            raise HTTPException(status_code=500, detail="Sürücü oluşturuldu fakat geri getirilemedi.")
            
        logger.info(f"Driver processed via Service: {created_sofor.ad_soyad} by {current_admin.kullanici_adi}")
...
async def download_template(
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=sofor_yukleme_sablonu.xlsx"}
    )
...
async def export_drivers(
    ...
    try:
        ...
        filename = f"sofor_listesi_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
...
async def update_sofor(
    ...
    try:
        ...
        if not success:
             raise HTTPException(status_code=404, detail="Şoför bulunamadı veya güncellenemedi")
...
async def update_driver_score(
    ...
    try:
        ...
        if not success:
             raise HTTPException(status_code=404, detail="Şoför bulunamadı veya puan güncellenemedi")
