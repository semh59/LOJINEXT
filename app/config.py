...
class Settings(BaseSettings):
    ...
    ADMIN_PASSWORD: SecretStr  # REQUIRED - repr()'da gizlenir
    ...
    def check_cors_wildcard(cls, v: List[str]) -> List[str]:
        """Zero Tolerance: Prod ortamında '*' yasak"""
        import os
    ...
    AI_MODEL_NAME: str = "qwen1_5-1_8b-chat-q4_k_m.gguf"
    AI_MODEL_URL: str = "https://huggingface.co/Qwen/Qwen1.5-1.8B-Chat-GGUF/resolve/main/qwen1_5-1_8b-chat-q4_k_m.gguf"
    ...
    # HGV Route Parametreleri (route_service.py'den taşındı)
    ...
    # AI Model Config (ai_service.py'den taşındı)
    AI_CONTEXT_WINDOW: int = 4096
    AI_THREADS: int = 4
    AI_GPU_LAYERS: int = 0  # 0 = CPU-only, -1 = tüm GPU
    AI_TEMPERATURE: float = 0.3
    AI_MODEL_SHA256: str = ""  # Model checksum doğrulaması için
    ...
    LOG_PII_SENSITIVE_KEYS: List[str] = [
        'password', 'passwd', 'sifre', 'secret', 'token', 
        'api_key', 'apikey', 'bearer', 'auth', 'credential', 
        'jwt', 'session_id'
    ]
