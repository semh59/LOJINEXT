...
class TestFAISSVectorStore:
    ...
    def test_search_with_filter(self, store):
        ...
        for i in range(3):
            embedding = np.random.rand(384).astype(np.float32)
            store.add(f'vehicle_{i}', f'Vehicle {i}', embedding, {'source_type': 'vehicle', 'source_id': i})
        
        for i in range(3):
            embedding = np.random.rand(384).astype(np.float32)
            store.add(f'driver_{i}', f'Driver {i}', embedding, {'source_type': 'driver', 'source_id': i})
...
class TestRAGEngine:
    ...
    async def test_bulk_index(self, engine):
        ...
        vehicles = [
            {'id': i, 'plaka': f'34ABC{i:03d}'} for i in range(3)
        ]
        drivers = [
            {'id': i, 'ad_soyad': f'Şoför {i}'} for i in range(2)
        ]
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not (SENTENCE_TRANSFORMERS_AVAILABLE and FAISS_AVAILABLE), 
    ...
    async def test_full_rag_pipeline(self):
        ...
        # Veri indeksle
        vehicles = [
            {'id': 1, 'plaka': '34ABC001', 'marka': 'Volvo', 'model': 'FH', 'hedef_tuketim': 28.0},
            {'id': 2, 'plaka': '06DEF002', 'marka': 'Mercedes', 'model': 'Actros', 'hedef_tuketim': 30.0},
        ]
